...
def get_rows_by_ids(conn, cursor, table, id_list):
    """
    ...
    query = f"SELECT * FROM {table} WHERE id IN ({','.join(map(str, ids_tuple))})"
            ...
            #for i, hz in zip([441], [100]):
                ...
                for timeframe_value in timeframe_values.keys():
                    ...
                    for ica in ica_types.keys():
                        ...
                        '''
                        values_dict = {"id": float(i), "db": float(db), "subject": float(subject), "exercise": float(exercise), "restimulus": float(restimulus),
