...
def check_table_exists(conn, cursor, table_name):
    try:
        # Ejecutar consulta para verificar la existencia de la tabla
        cursor.execute(f"SHOW TABLES LIKE '{table_name}'")
        result = cursor.fetchone()
        
        # Si result no es None, la tabla existe
...
def create_table(conn, cursor, table):
    ...
    def emg_features_str():

        return_str = ''

        for i in range(10):
            return_str = return_str + f'emg{i}_std_cA FLOAT NOT NULL,\n'
            return_str = return_str + f'emg{i}_std_cD FLOAT NOT NULL,\n '
            return_str = return_str + f'emg{i}_var_cA FLOAT NOT NULL,\n '
            return_str = return_str + f'emg{i}_var_cD FLOAT NOT NULL,\n '
            return_str = return_str + f'emg{i}_wmav_cA FLOAT NOT NULL,\n '
            return_str = return_str + f'emg{i}_wmav_cD FLOAT NOT NULL,\n '
...
async def asyncio_mariadb_insert_into_table(data, host = 'localhost', port = 3306, user = "root", password = 'Once062002!', database = 'TFG', table = "tanhICAmWDT"):
    ...
    async with pool.acquire() as conn:
        async with conn.cursor() as cursor:
            ...
            # Insert rows in batches
            batch_ins_size = 10000  # Adjust the batch size based on your needs
            batch = []
...
def describe_maria_table(conn, cursor, table):

    # Obtener el esquema de la tabla usando DESCRIBE (o SHOW COLUMNS FROM)
    cursor.execute(f"DESCRIBE {table}")
    schema_info = cursor.fetchall()
    schema = [(col[0], col[1].upper()) for col in schema_info]
    return schema
...
def process_windows(raw_conn, raw_cursor):
    ...
    time_conn = mysql.connector.connect(
                host='localhost',
                user='root',
                password='Once062002!',
                database='TFG'
    ...
    if not check_table_exists(time_conn, time_cursor, ica_types['tanh']):
        create_table(time_conn, time_cursor, ica_types['tanh'])

    #### MAX ID FOR THE WINDOW DB ####
    max__id = query_max_value(time_conn, time_cursor, ica_types['tanh'], '_id')
    win_schema = describe_maria_table(time_conn, time_cursor, ica_types['tanh'])
    ...
    for b in range(0, len(array), batch_size):
        ...
        asyncio.run(asyncio_mariadb_insert_into_table(values_dict))
        #input("INPUT")
